...
- (BOOL)registerUserModelsForLanguageCode:(NSString*)languageCode {
  ...
  NSArray* languageCodeUserModelIds =
      [self.userModelPaths allKeysForObject:isoLanguageCode];
  if ([languageCodeUserModelIds count] == 0) {
    return NO;
  }

  NSString* languageCodeUserModelId = [languageCodeUserModelIds firstObject];
  BLOG(1, @"Registering Brave User Model Installer (%@) with id %@",
       languageCode, languageCodeUserModelId);

  BLOG(1, @"Notifying user model observers");
  const std::string bridged_language_code_user_model_idkey =
      languageCodeUserModelId.UTF8String;
...
- (BOOL)registerUserModelsForCountryCode:(NSString*)countryCode {
  ...
  NSArray* countryCodeUserModelIds =
      [self.userModelPaths allKeysForObject:isoCountryCode];
  ...
  BLOG(1, @"Registering Brave User Model Installer (%@) with id %@",
       countryCode, countryCodeUserModelId);

  BLOG(1, @"Notifying user model observers");
  const std::string bridged_country_code_user_model_idkey =
      countryCodeUserModelId.UTF8String;
...
- (void)registerUserModels {
  ...
  if (![self registerUserModelsForLanguageCode:currentLocale.languageCode]) {
    BLOG(1, @"%@ not supported for user model installer",
...
- (void)periodicallyCheckForUserModelUpdates {
  const uint64_t time_offset =
      6 * 60 * 60;  // every 6 hours and on browser launch

  const auto __weak weakSelf = self;
  self.updateUserModelTimer = [NSTimer
...
- (void)updateUserModels {
  ...
  for (NSString* key in dict) {
    ...
    const auto __weak weakSelf = self;
    [self downloadUserModelForId:key
...
- (void)downloadUserModelForId:(NSString*)id
                    completion:
  ...
  NSString* manifestUrl =
      [baseUrl stringByAppendingPathComponent:@"models.json"];
  return [self.commonOps loadURLRequest:manifestUrl.UTF8String
      headers:{}
      content:""
      content_type:""
      method:"GET"
      callback:^(const std::string& errorDescription, int statusCode,
        ...
        if ([schemaVersion intValue] !=
            kCurrentUserModelManifestSchemaVersion) {
          completion(NO, YES);
          return;
        }
        ...
        for (NSDictionary* model in models) {
          ...
          if (version <= userModelMetadataDict[modelId]) {
            BLOG(1, @"%@ user model is up to date on version %@", modelId,
                 version);
            completion(YES, NO);
            return;
          }
          ...
          return [strongSelf.commonOps loadURLRequest:modelUrl.UTF8String
              headers:{}
              content:""
              content_type:""
              method:"GET"
              callback:^(
                ...
                if (statusCode != 200) {
                  BLOG(1, @"Failed to download %@ user model version %@",
                       modelId, version);
                  completion(NO, YES);
                  return;
                }
...
- (void)loadUserModelForId:(const std::string&)id
  ...
  const auto __weak weakSelf = self;
  [self
      downloadUserModelForId:bridgedId
                  completion:^(BOOL success, BOOL shouldRetry) {
                    const auto strongSelf = weakSelf;
                    if (!strongSelf || ![strongSelf isAdsServiceRunning]) {
                      return;
                    ...
                    if (!success || contents.empty()) {
                      if (shouldRetry) {
                        ...
                        formatter.dateStyle = NSDateFormatterFullStyle;
                        formatter.timeStyle = NSDateFormatterFullStyle;
                        BLOG(1, @"Retry loading %@ user model on %@", bridgedId,
                             [formatter
                                 stringFromDate:
                                     [[NSDate date]
                        ...
                        dispatch_after(
                            ...
                            dispatch_get_main_queue(), ^{
                              ...
                              [strongSelf2
                                  loadUserModelForId:bridgedId.UTF8String
                                            callback:callback];
                            });
                      }
